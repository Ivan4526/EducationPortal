...
namespace Repository
{
    ...
    {
        string connectionString = @"Data Source=.\MSSQLSERVER01;Initial Catalog=NixDatabase;Integrated Security=True";
        ...
        {
            int userId = 0;
            string sqlExpression = $"select Id from Users where id = {id}";
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                connection.Open();
                SqlCommand command = new SqlCommand(sqlExpression, connection);
