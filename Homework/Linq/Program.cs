        ...
        List<Customer> customers = new List<Customer>
{
    ...
    new Customer(20, "Anna Ledesma", new DateTime(2017, 12, 29), 0.8)
        ...
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("First registration: " + p.FindFirstRegistration());
            Console.WriteLine("Average balance: " + p.ComputeMiddleBalance());
        }
        public Customer FindFirstRegistration()
        {
            return customers.OrderBy(p => p.RegistrationDate).FirstOrDefault();
        }
        public double ComputeMiddleBalance()
        {
            return customers.Average(p => p.Balance);
        }
        public Customer FindByRegisterDate(Func<Customer, bool> predicate)
        {
            return customers.Where(predicate).FirstOrDefault();
        }
        public IEnumerable<Customer> FilterById()
        {
            var result = customers.GroupBy(p => p.Id).SelectMany(x => x);
            return result;
        }
        public string ShowCustomersNames()
        {
            StringBuilder builder = new StringBuilder(100);
            var names = customers.Select(p => p.Name);
